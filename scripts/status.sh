...
ps -axo rss,comm 2>/dev/null | awk '/mlx_lm|python.*server|open_webui|open-webui/ {sum+=$1} END {printf "  관련 프로세스 RSS 합계: %.1f GB\n", sum/1024/1024}'

hr "Tailscale"
command -v tailscale >/dev/null 2>&1 && tailscale status 2>/dev/null | head -5 || echo "  tailscale CLI 없음/미연결"

hr "최근 로그 (tail)"
for f in "$LOG_DIR"/model-server.err.log "$LOG_DIR"/webui.err.log; do
  [ -f "$f" ] && { echo "--- $f"; tail -3 "$f"; }
done
