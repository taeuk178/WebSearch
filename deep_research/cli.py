...
from .policy import ARXIV_REQUEST_INTERVAL_SECONDS
from .reader import HttpPageReader, PageReader, SnippetReader
from .search import ArxivSearchProvider, BraveSearchProvider, MockSearchProvider, SearchProvider, SearchProviderError
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Minimum seconds between arXiv API or paper page requests.",
    )
    parser.add_argument(
        "--read-pages",
        action="store_true",
...
def _build_search_provider(args: argparse.Namespace) -> SearchProvider:
    ...
    if args.search_provider == "arxiv":
        return ArxivSearchProvider(timeout=args.search_timeout, request_interval_seconds=args.arxiv_delay)
    return MockSearchProvider()


def _build_page_reader(args: argparse.Namespace) -> PageReader:
    if args.read_pages:
        return HttpPageReader(timeout=args.read_timeout, arxiv_interval_seconds=args.arxiv_delay)
