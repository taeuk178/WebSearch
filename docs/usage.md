...
- **매 질문이 자동으로 웹 검색을 탄다.** UI 토글과 무관하게, 전역 Filter 함수
  `force_web_search`(`webui/functions/force_web_search.py`)가 서버에서 모든 요청에
  `features.web_search=True`를 강제 주입한다. (미들웨어가 features를 읽기 전 inlet 단계)
  - DuckDuckGo에서 최대 5개 결과의 제목·URL·스니펫을 모델에 직접 전달.
  - 답변에 **클릭 가능한 출처**와 인용 번호가 붙는다.
  - 결과가 부족하거나 출처가 상충하면 모델이 그 사실을 밝히고 단정하지 않는다.
- ⚠️ 서버 강제라서, 입력창의 웹 검색 토글이 시각적으로 꺼져 보여도 **실제로는 항상 검색된다.**
  (토글 표시는 브라우저 localStorage 캐시 영향. 표시를 맞추려면 완전 로그아웃/재로그인.)
- 순수 대화(검색 끄기)가 필요하면 `force_web_search` 필터를 잠시 비활성화한다:
  설정 > 관리자 > 함수 > Force Web Search 토글 OFF (또는 API `.../toggle`).
